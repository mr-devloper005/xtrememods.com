...
export function EditableContactLeadForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-6 rounded-2xl border border-[var(--editable-border)] bg-[var(--slot4-panel-bg)] p-6 md:p-8">
      ...
      {message ? (
        <div className={`mt-5 flex items-start gap-3 rounded-xl px-4 py-3 text-sm font-semibold ${status === 'success' ? 'bg-emerald-50 text-emerald-700' : 'bg-[var(--slot4-accent-soft)] text-[var(--slot4-accent)]'}`}>
      ...
      <button
        ...
        className="mt-6 inline-flex h-12 w-full items-center justify-center gap-2 rounded-full bg-[var(--slot4-accent-fill)] px-6 text-sm font-semibold tracking-[0.04em] text-[var(--slot4-on-accent)] transition hover:bg-[var(--slot4-accent-strong)] disabled:cursor-not-allowed disabled:opacity-70"
      >
